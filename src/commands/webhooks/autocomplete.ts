...
export const webhookAutocomplete: AppCommandAutocompleteCallback = async (ctx) => {
  ...
  const guildWebhooks = await ctx.client.get(Routes.guildWebhooks(ctx.interaction.guild_id!)) as APIWebhook[];
  ...
  return matches.map(w => ({
    name: `${w.name}${w.user ? ` | ${w.user.global_name ?? w.user.username}` : ''}`.slice(0, 100),
    value: w.id,
  }));
}
