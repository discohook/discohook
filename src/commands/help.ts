...
const tags: Record<string, string | APIEmbed> = {
  ...
  channel: {
    ...
    description: "To mention a channel or thread, invite the Discohook bot at https://discohook.app/bot and use the **/format channel** command. Point it to any channel and it will respond with the appropriate code to tag it!"
  },
  emoji: {
  ...
  image: {
    ...
    description: "Make sure the URL you used for the image is a proper image URL. It may have happened that you copied a link to a webpage that contains the image, instead of the image itself.\n\nTo make sure images load reliably, upload the images to a private Discord channel. Then copy the image link from Discord and paste it in Discohook. If done properly the image should appear in Discohook's preview and send to Discord without issues."
  },
  buttons: {
    title: "How do I add buttons to my messages/embeds?",
    description: "Use the `/buttons` command or the \"Add Button\" message command to add buttons to your messages. Messages must be sent by a webhook that Discohook Utils#4333 created in order for this to work. To get a webhook created by Discohook Utils#4333, use `/webhook create`.\n\n[Here's an example of the \"Add Button\" message command](https://www.youtube.com/watch?v=tGQsZaIGr2A&list=PL2lbsZZaSX2heZ_bGhkJ3WJHm9PijiXWv)"
  },
  sidebar: {
    title: "How do I remove the sidebar color on an embed?",
    description: "Set the \"Color\" option to #2f3136 in the body section. For light mode, use #f2f3f5.",
    color: 3092790,
    image: {
...
export const helpEntry: ChatInputAppCommandCallback = async (ctx) => {
  ...
  return ctx.reply({
    content: "No tag found. Select an item from the autocomplete menu or use the exact title of a valid item.",
    flags: MessageFlags.Ephemeral,
  });
}
...
export const helpAutocomplete: AppCommandAutocompleteCallback = async (ctx) => {
  ...
  if (!embed) {
    const entries = Object
      ...
      .filter(v => v[1].title && v[1].title.toLowerCase().includes(query.value.toLowerCase().trim()));
