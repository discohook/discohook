...
- If you have Send Message or Send Webhook Message actions:
  - Ensure the message is valid. You may have attached a local file without uploading it to a filehosting service (see [Attachments & Files](discohook://guide/troubleshooting/attachments)), or you may simply have an invalid URL/too many characters. If you can't find anything wrong with it, join the [support server](discohook://discord) and we'll take a look.
  ...
  - Only if you are using the Send Message action (not webhook):
    - If you want to send a message in a thread, create a variable with the name `channelId` whose value is the ID of the thread.
